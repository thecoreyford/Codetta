/*
  ==============================================================================
    ...
    EndRepeat.h
    Version Two Created: 9 Oct 2019 12:26:51pm
    Author:  Corey Ford
    ...
    public:
        ...
        RepeatModel()
        {
            ...
            showArray.onClick = [this]
            {
                ...
                CallOutBox::launchAsynchronously (std::make_unique<RepeatView>(pitchShifts),
                                                  getScreenBounds(),
                                                  nullptr);
                //for (int i = 0; i < CODETTA_MAX_REPEATS; ++i)
                  //  DBG (pitchShifts[i]);
        ...
        void doAction() override
        {
            ...
            if (slider != nullptr)
            {
                ...
                for (int i = 0; i < (int)totalRepeats; ++i)
                {
                    ...
                    // should I deregister this block (if it not a start repeat)
                    doDeregister = !current->isStartBlock();
                    
                    if (current != nullptr)
                    {
                        // add arc drawings
                        LiveGuiPlayback::get().add (PlaybackSettings::get().getLatestTimestampAddition(),
                                                              {getY(),
                                                               getX() + (getWidth() / 2),
                                                               current->getX() + (current->getWidth() / 2),
                                                               1.0f } );
                ...
                if (doDeregister == true)
                    PlaybackSettings::get().deregisterStartRepeat(); //< I think
            }
        }
